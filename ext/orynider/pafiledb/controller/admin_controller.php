...
class admin_controller
{
	...
	*/
	public function __construct(
		\orynider\pafiledb\core\functions $functions,
	...
	public function display_config()
	{
		...
		if ($set_pagination)
		{
			...
			// Update values
			$sql = 'UPDATE ' . $this->pa_config_table . ' SET ' . $this->db->sql_build_array('UPDATE', $sql_ary);
		...
		else
		{
			$this->template->assign_vars(array(
				'PAGINATION_ACP'		=> $pafiledb_config['pagination_acp'],
				'PAGINATION_USER'		=> $pafiledb_config['pagination_user'],
				'ANNOUNCE_ENABLE'		=> $pafiledb_config['announce_enable'],
				'ANNOUNCE_FORUM'		=> $pafiledb_config['announce_forum'],
				'ANNOUNCE_LOCK'			=> $pafiledb_config['announce_lock_enable'],
	...
	public function add_new()
	{
		...
		$upload_dir = 'ext/orynider/pafiledb/files/' . $target;
		...
		if ($pafiledb_config['announce_enable'] == 1)
		{
			$sql = 'SELECT *
				FROM ' . $this->pa_cat_table . '
			...
			$this->functions->create_announcement($download_subject, $download_msg, $pafiledb_config['announce_forum']);
		}
		...
		if (!$ftp_upload)
		{
			...
			else
			{
				...
				if ($current_cat_id != $v_cat_id)
				{
					...
					{
						// Announce download, if enabled
						if ($pafiledb_config['announce_enable'] == 1 && $announce_up != '')
						{
							$sql = 'SELECT *
								FROM ' . $this->pa_cat_table . '
								WHERE cat_id = ' . (int) $v_cat_id;
						...
						if (rename(($this->root_path . 'ext/orynider/pafiledb/files/' . $current_cat_name . '/' .$new_filename), ($this->root_path . 'ext/orynider/pafiledb/files/' . $cat_dir . '/' . $new_filename)))
						{
							$this->db->sql_query('UPDATE ' . $this->pa_files_table . ' SET ' . $this->db->sql_build_array('UPDATE', $sql_ary) . ' WHERE download_id = ' . (int) $file_id);
	...
	public function display_downloads()
	{
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			$this->template->assign_block_vars('downloads', array(
				...
				'CATNAME'		=> $row['cat_name'],
				'U_COPY'		=> $this->u_action . '&amp;action=copy_new&amp;file_id=' .$row['download_id'],
				'U_EDIT'		=> $this->u_action . '&amp;action=edit&amp;file_id=' .$row['download_id'],
				'U_DEL'			=> $this->u_action . '&amp;action=delete&amp;file_id=' .$row['download_id'],
			));
		}
		$this->db->sql_freeresult($result);
	...
	*/
	public function manage_cats()
	{
		...
		else
		{
			...
			$pa_files_nav = $this->functions->get_cat_branch($parent_id, 'parents', 'descending');
		...
		for ($i = 0; $i < count($pa_files); $i++)
		{
			...
			$url = $this->u_action . "&amp;parent_id=$parent_id&amp;cat_id={$pa_files[$i]['cat_id']}";

			$this->template->assign_block_vars('catrow', array(
				'FOLDER_IMAGE'			=> $folder_image,
				'U_CAT'					=> $this->u_action . '&amp;parent_id=' . $pa_files[$i]['cat_id'],
				'CAT_NAME'				=> $pa_files[$i]['cat_name'],
				'CAT_SUBS'				=> ($pa_files[$i]['left_id'] + 1 == $pa_files[$i]['right_id'] && !$pa_files[$i]['cat_id'] == $pa_files[$i]['parent_id']) ? true : false,
				'CAT_SUBS_SHOW'			=> ($pa_files[$i]['left_id'] + 1 != $pa_files[$i]['right_id'] && $pa_files[$i]['cat_id'] != $parent_id	|| $pa_files[$i]['parent_id'] == 0) ? true : false,
	...
	*/
	public function create_cat()
	{
		if ($this->request->is_set('submit'))
		{
			...
			if ($pa_files_data['parent_id'])
			{
				$sql = 'SELECT left_id, right_id
					FROM ' . $this->pa_cat_table . '
					WHERE cat_id = ' . $pa_files_data['parent_id'];
				$result = $this->db->sql_query($sql);
				$row = $this->db->sql_fetchrow($result);
				$this->db->sql_freeresult($result);
			...
			if (is_dir($this->root_path . 'ext/orynider/pafiledb/files/' . $cat_sub_dir_name))
			{
				trigger_error($this->user->lang['ACP_CAT_NEW_DONE'] . adm_back_link($this->u_action . '&amp;parent_id=' . $pa_files_data['parent_id']));
			}
			else if (mkdir(($this->root_path . 'ext/orynider/pafiledb/files/' . $cat_sub_dir_name)))
			{
				if (copy(($this->root_path . 'ext/orynider/pafiledb/files/' .'index.htm'), ($this->root_path . 'ext/orynider/pafiledb/files/' . $cat_sub_dir_name . '/index.htm')))
				{
					trigger_error($this->user->lang['ACP_CAT_NEW_DONE'] . adm_back_link($this->u_action . '&amp;parent_id=' . $pa_files_data['parent_id']));
				}
			}
	...
	*/
	public function edit_cat()
	{
		...
		if ($this->request->is_set('submit'))
		{
			...
			if ($row['parent_id'] != $pa_files_data['parent_id'])
			{
				...
				else
				{
					...
					$parent = $this->functions->get_cat_info($pa_files_data['parent_id']);
					//left_id
					$sql = 'UPDATE ' . $this->pa_cat_table . '
						SET left_id = left_id + ' . $moving_ids . '
		...
		$parents_list = $this->functions->make_cat_select($pa_files_data['parent_id'], $cat_id);
